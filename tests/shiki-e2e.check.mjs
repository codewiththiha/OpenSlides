...
const raw = h.codeToTokensBase(CODE, { lang: "typescript", theme: "github-dark" });
const codeLines = CODE.split("\n");
if (raw.length !== codeLines.length) {
  throw new Error(`token line split mismatch: ${raw.length} vs ${codeLines.length}`);
}
...
for (const l of plan.lines) console.log(l.lineIndex, `[${l.startChar},${l.endChar})`, l.html);
console.log("eraser:", plan.eraserColor, "| selected:", JSON.stringify(plan.selectedText));

if (!plan.lines[0].html.includes("&lt;")) throw new Error("entity lost in clone (line 0)");
if (plan.lines[0].html.includes("&") && /&(?!amp;|lt;|gt;|quot;)/.test(plan.lines[0].html.replace(/&amp;|&lt;|&gt;|&quot;/g, ""))) {
  throw new Error("raw & leaked into clone html");
}
if (!plan.lines[1].html.includes("&quot;")) throw new Error("quote entity lost in clone (line 1)");
