...
const whatsNewBlock = fields.match(/^whatsNew:\s*\|\s*\r?\n((?:^[ \t]+.*(?:\r?\n|$))*)/m)?.[1]
const whatsNewFromHeading = content.match(/##\s*What['’]s New\s*\r?\n([\s\S]*?)(?=\r?\n##\s|$)/i)?.[1]
const whatsNew = (whatsNewBlock ?? whatsNewFromHeading ?? content.split(/\r?\n\r?\n/)[0] ?? '')
  .split(/\r?\n/)
