...
export function useHighlightCrud({
  ...
  const clearPreview = useUiStore((s) => s.clearPreviewHighlightSetting);
  ...
  const updateHighlight = useCallback((id: string, patch: Partial<Highlight>) => { if (!slideId) return; mutation.mutate({ slideId, payload: { highlights: highlights.map((h) => h.id === id ? { ...h, ...patch } : h) } }, { onSuccess: () => clearPreview(id) }); }, [slideId, highlights, mutation, clearPreview]);
  const deleteHighlight = useCallback((id: string) => { if (!slideId) return; const index = highlights.findIndex((h) => h.id === id); mutation.mutate({ slideId, payload: { highlights: highlights.filter((h) => h.id !== id) } }, { onSuccess: () => clearPreview(id) }); if (expandedId === id) setExpandedId(null); if (index >= 0) { if (previewIndex === index) setPreviewIndex(-1); else if (previewIndex > index) setPreviewIndex(previewIndex - 1); } }, [slideId, highlights, mutation, clearPreview, expandedId, previewIndex, setPreviewIndex]);
